...
/*   Updated: 2025/07/03 19:31:04 by houarrak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_args(t_rules *rules, int ac, char **av)
{
	...
	rules->nb_meals_required = (ac == 6) ? ft_atol(av[5]) : 0;
	rules->simulation_end = 0;
	if (rules->nb_philo < 1 || rules->time_to_die < 0
		|| rules->time_to_eat < 0 || rules->time_to_sleep < 0
		|| (ac == 6 && rules->nb_meals_required < 1))
		return (1);
	return (0);
}
...
static void assign_forks_and_philo(t_rules *rules, int i)
{
    ...
    if (i == rules->nb_philo - 1)
        rules->philos[i].fork_order = 1; //right first
    else
        rules->philos[i].fork_order = 0; // left first
}
